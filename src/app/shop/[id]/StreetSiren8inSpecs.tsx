...
export function StreetSiren8inSpecs({ slug }: { slug: string }) {
  if (slug !== 'street-siren-8in' && slug !== 'street-siren') return null;

  const BASE = '/images/products/street-siren-8in';
      ...
      <div className="max-w-5xl mx-auto px-6 flex flex-col gap-0">
        {PAGES.map((page) => (
          <div key={page.num} className={`w-full bg-white relative ${page.num === PAGES.length ? 'overflow-hidden' : ''}`} style={page.num === PAGES.length ? { aspectRatio: '1 / 0.45' } : {}}>
            <img
              src={page.src}
              alt={`Street Siren Product Documentation - Page ${page.num}`}
