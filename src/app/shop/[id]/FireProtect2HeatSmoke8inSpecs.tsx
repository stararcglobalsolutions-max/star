...
export function FireProtect2HeatSmoke8inSpecs({ slug }: { slug: string }) {
  if (slug !== 'fire-protect-2-heat-smoke-8in' && slug !== 'fire-protect-2-heat-smoke') return null;

  const BASE = '/images/products/fire-protect-2-heat-smoke-8in';
  ...
  const PAGES: { num: number; src: string }[] = [
    ...
    { num: 5, src: `${BASE}/hires_page_5.png?v=15` },
    { num: 6, src: `${BASE}/hires_page_6.png?v=15` },
    { num: 7, src: `${BASE}/hires_page_7.png?v=15` },
  ];
      ...
      <div className="max-w-5xl mx-auto px-6 flex flex-col gap-0">
        ...
        {PAGES.map((page) => (
          <div key={page.num} className={`w-full bg-white relative ${page.num === PAGES.length ? 'overflow-hidden' : ''}`} style={page.num === PAGES.length ? { aspectRatio: '1 / 0.45' } : {}}>
            <img
              src={page.src}
              alt={`Fire Protect 2 (Heat/Smoke) Product Documentation - Page ${page.num}`}
