...
export function KeypadTouchScreen8inSpecs({ slug }: { slug: string }) {
  if (slug !== 'keypad-touchscreen-8in' && slug !== 'keypad-touchscreen') return null;

  const BASE = '/images/products/keypad-touchscreen-8in';
      ...
      <div className="max-w-5xl mx-auto px-6 flex flex-col gap-0">
        {PAGES.map((page) => (
          <div key={page.num} className={`w-full bg-white relative ${page.num === PAGES.length ? 'overflow-hidden' : ''}`} style={page.num === PAGES.length ? { aspectRatio: '1 / 0.45' } : {}}>
            <img
              src={page.src}
              alt={`Keypad TouchScreen Product Documentation - Page ${page.num}`}
