...
export function MotionCamOutdoorPhod8inSpecs({ slug }: { slug: string }) {
  if (slug !== 'motioncam-outdoor-phod-8in' && slug !== 'motioncam-outdoor-phod') return null;

  const BASE = '/images/products/motioncam-outdoor-phod-8in';
      ...
      <div className="max-w-5xl mx-auto px-6 flex flex-col gap-0">
        ...
        {PAGES.map((page) => (
          <div key={page.num} className={`w-full bg-white relative ${page.num === PAGES.length ? 'overflow-hidden' : ''}`} style={page.num === PAGES.length ? { aspectRatio: '1 / 0.45' } : {}}>
            <img
              src={page.src}
              alt={`MotionCam Outdoor (PhOD) Product Documentation - Page ${page.num}`}
